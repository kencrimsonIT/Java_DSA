...
public class MyArray {
    ...
    public int[] getMissingValues() {
        int missingCount = 0; //Number of missing values
        for (int i = 1; i < array.length; i++) {
        ...
        int[] result = new int[missingCount]; //Missing values array
        int index = 0;
        for (int i = 1; i < array.length; i++) {
            int diff = array[i] - array[i - 1];
            if (diff > 1) {
                for (int missingValue = array[i - 1] + 1; missingValue < array[i]; missingValue++) {
                    result[index++] = missingValue;
                }
            }
        }
    ...
    static void main(String[] args) {
        ...
        int[] test4 = array4.getMissingValues();
        int[] test5 = array5.getMissingValues();
